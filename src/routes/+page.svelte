<!--
  @component
  Description: Landing page with the hero section, lab logo, and "how to" walkthrough steps.

  @author Grace Steinmetz <gesparkles@gmail.com>
  @contributors K. Seow <kseow@wisc.edu>
  @created 2025-04-01
  @version 0.2.0
  @license MIT
-->
<script lang="ts">
	import { base, asset } from '$app/paths';
	type Step = { title: string; description: string };

	let steps: Step[] = [
		{
			title: 'Drag & Drop Your Audio',
			description:
				'Drop one or more .wav files into the tool. Everything runs locally—no files are uploaded.'
		},
		{
			title: 'Choose Your Visualization',
			description:
				'Pick between waveform, spectrogram, or both to explore your audio data visually.'
		},
		{
			title: 'Adjust Your Settings',
			description: 'Toggle Sliders to fine-tune amplitude, time, and frequency.'
		},
		{
			title: 'Download Your Graph',
			description: 'Download as .png .jpg .svg files when you are ready.'
		}
	];
</script>

<section class="px-6 py-16 text-gray-800">
	<div class="mx-auto flex max-w-4xl flex-col items-center text-center">
		<img src={asset('/bicklab_logo.png')} alt="BickLab Logo" class="mb-6 h-auto w-40" />

		<h1 class="mb-4 text-4xl font-bold tracking-tight md:text-5xl">
			Welcome to <span class="text-purple-400">Bick Graphing</span>
		</h1>

		<p class="mb-10 text-lg text-gray-600 md:text-xl">
			A clean and simple offline graphing tool for exploring your audio.
		</p>

		<div class="flex flex-col justify-center gap-4 md:flex-row">
			<a
				href="{base}/graphing"
				class="rounded-2xl bg-purple-500 px-6 py-3 text-sm font-semibold text-white shadow transition hover:bg-purple-600"
			>
				Try It Now
			</a>

			<a
				href="#howto"
				class="rounded-2xl border border-purple-400 bg-white px-6 py-3 text-sm font-semibold text-purple-600 shadow transition hover:bg-purple-50"
			>
				Learn How It Works
			</a>
		</div>
	</div>
</section>

<section id="howto" class="rounded-xl border-t border-gray-200 bg-gray-50 px-6 py-14 text-gray-800">
	<div class="mx-auto max-w-3xl">
		<h2 class="mb-4 text-center text-3xl font-bold">How to Use</h2>

		<div class="grid gap-10">
			{#each steps as step, i (step)}
				<div class="flex gap-4">
					<div
						class="flex h-12 w-12 items-center justify-center rounded-full bg-purple-100 font-bold text-purple-600 shadow"
					>
						{i + 1}
					</div>
					<div>
						<h3 class="text-xl font-semibold">{step.title}</h3>
						<p class="mt-2 text-gray-600">{step.description}</p>
					</div>
				</div>
			{/each}
		</div>
	</div>
</section>

<section class=" px-6 py-14 text-gray-800">
	<div class="mx-auto max-w-5xl text-center">
		<h2 class="mb-12 text-3xl font-bold text-gray-900">Why Use Bick Graphing?</h2>
		<div class="grid grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
			<div
				class="flex flex-col items-center rounded-xl bg-white p-6 shadow-md transition hover:shadow-lg"
			>
				<div
					class="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-violet-100 text-xl text-violet-600"
				>
					🔒
				</div>
				<h3 class="mb-2 text-lg font-semibold text-gray-900">Private & Local</h3>
				<p class="text-sm text-gray-500">
					All processing happens in your browser—no files are uploaded.
				</p>
			</div>

			<div
				class="flex flex-col items-center rounded-xl bg-white p-6 shadow-md transition hover:shadow-lg"
			>
				<div
					class="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-violet-100 text-xl text-violet-600"
				>
					⚡
				</div>
				<h3 class="mb-2 text-lg font-semibold text-gray-900">No Setup Needed</h3>
				<p class="text-sm text-gray-500">Just drag and drop a .wav file to begin instantly.</p>
			</div>

			<div
				class="flex flex-col items-center rounded-xl bg-white p-6 shadow-md transition hover:shadow-lg"
			>
				<div
					class="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-violet-100 text-xl text-violet-600"
				>
					🧪
				</div>
				<h3 class="mb-2 text-lg font-semibold text-gray-900">Built for Research</h3>
				<p class="text-sm text-gray-500">Designed with lab and field audio analysis in mind.</p>
			</div>
		</div>
	</div>
</section>

<section class="rounded-xl bg-white px-6 py-12 text-gray-800">
	<div class="mx-auto max-w-4xl">
		<h2 class="mb-12 text-center text-3xl font-bold text-gray-900">FAQs</h2>

		<div class="space-y-8">
			<div>
				<h3 class="flex items-start gap-2 text-lg font-semibold text-gray-900">
					<span class="text-xl text-purple-500">❓</span>
					Is this tool free to use?
				</h3>
				<p class="mt-2 text-sm text-gray-600">
					Yes, Bick Graphing is completely free and runs entirely in your browser.
				</p>
			</div>

			<div>
				<h3 class="flex items-start gap-2 text-lg font-semibold text-gray-900">
					<span class="text-xl text-purple-500">❓</span>
					Can I use this offline?
				</h3>
				<p class="mt-2 text-sm text-gray-600">
					Absolutely. No data leaves your device, and the tool works offline after the initial load.
				</p>
			</div>
		</div>

		<div class="mt-8 text-center">
			<a
				href="{base}/faq"
				class="inline-block rounded-xl border border-purple-500 px-5 py-2 text-sm font-semibold text-purple-600 transition hover:bg-purple-50"
			>
				View All FAQs
			</a>
		</div>
	</div>
</section>
