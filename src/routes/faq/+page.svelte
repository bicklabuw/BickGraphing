...
<details class="group/nerds mt-3 rounded-lg border border-purple-200 bg-purple-50 p-3 open:bg-purple-100">
	...
	<div class="mt-3 space-y-2 text-xs text-gray-700">
		...
		{
			question: 'What does the Auto-update Spectrogram toggle do?',
			answer: `The Auto-update Spectrogram toggle controls what happens to the spectrogram when you drag the time or frequency sliders. With Auto-update <strong>on</strong> (the default), the spectrogram re-runs its <a href="#how-long" class="text-purple-600 underline hover:text-purple-800">STFT pipeline</a> for the new bounds, so the image is freshly computed at the resolution of the visible window. With Auto-update <strong>off</strong>, slider drags zoom into the most recently computed spectrogram; the response is near-instant, but very close zoom levels can look pixelated because you are scaling up pixels that were rendered for a wider view.<br><br>When Auto-update is off and you want full resolution at the current zoom, click the amber <strong>Refresh Graph</strong> button (to the left of Reset Changes in each spectrogram's action row). It re-runs the STFT once for the visible bounds without changing the toggle state. For quick exploration, off is faster and feels more responsive; flip on or click Refresh Graph before <a href="#what-download-formats-are-supported" class="text-purple-600 underline hover:text-purple-800">downloading</a> a spectrogram so the saved file is sharpest.`
		},
		{
